# 免费代理订阅服务

> 自动抓取、测速、转换格式的免费代理订阅服务

## 功能特性

- 🌐 **多源抓取** - 自动从多个公开代理列表抓取
- ⚡ **双重测速** - 延迟测试 + 下载速度测试
- 🌍 **地区识别** - 使用 ip-api.com 批量查询 IP 地区
- 📊 **智能排序** - 综合延迟和速度计算分数排名
- 🔒 **安全筛选** - 黑名单过滤、排除高风险国家
- ⏰ **定时更新** - 支持 Cron 自动刷新
- 📱 **多格式输出** - 支持 Clash / JSON 格式

## 快速开始

### 访问订阅

```
https://your-worker.your-subdomain.workers.dev/
```

### 参数

| 参数 | 说明 | 示例 |
|------|------|------|
| `format` | 输出格式 | `clash`(默认), `json` |
| `country` | 筛选国家 | `US`, `JP`, `CN` |
| `limit` | 限制数量 | `10`, `20` |
| `key` | 访问密钥 | (可选) |

### 接口

| 路径 | 说明 |
|------|------|
| `/` | Clash 订阅 |
| `/?format=json` | JSON 格式 |
| `/?update=1` | 手动触发更新 |
| `/status` | 查看状态和测试详情 |

## 代理命名规则

代理按 **地区-分数排名** 命名：

```
US-01  → 综合分数最高的美国代理
US-02  → 分数第二高的美国代理
JP-01  → 综合分数最高的日本代理
...
```

**好处**：下次更新后名称不变，你选 US-01 就不用重新选

## 部署

### 1. 创建 Worker

1. 打开 [Cloudflare Dashboard](https://dash.cloudflare.com)
2. 进入 **Workers** → **创建 Worker**
3. 名称随意，如 `proxy-sub`
4. 粘贴 `worker.js` 代码
5. 点 **部署**

### 2. 创建 KV 存储

KV 用于缓存代理数据，必须创建：

1. 回到 Cloudflare 主页，搜索 **KV** 或进入 **Workers** → **KV**
2. 点击 **创建命名空间**
3. 名称填 `proxy-cache`，创建

### 3. 绑定 KV 到 Worker

1. Workers → 你的 Worker → **设置** → **变量**
2. 找到 **KV 命名空间绑定** → **添加绑定**
   - **变量名**: `PROXY_KV`
   - **KV 命名空间**: 选择刚才创建的 `proxy-cache`

### 4. 重新部署

绑定 KV 后点 **重新部署**

### 5. 配置 GitHub 同步（推荐）

1. Worker → **设置** → **Git** → **添加 GitHub 仓库**
2. 选择 `yingw/free-proxy-sub`
3. 以后代码更新会自动部署

### 6. 配置定时更新（可选）

1. Worker → **触发器** → **添加 Cron 触发器**
2. 填入 `0 * * * *`（每小时）

## 配置

编辑 `worker.js`：

```javascript
const CONFIG = {
  // ========== 数据源 ==========
  SOURCES: [
    'https://raw.githubusercontent.com/TheSpeedX/PROXY-List/master/http.txt',
  ],
  
  // ========== 测速配置 ==========
  TEST_URL: 'https://httpbin.org/get',
  SPEED_TEST_URL: 'https://speed.cloudflare.com/__down?bytes=1000000',
  TIMEOUT: 5000,
  MAX_PER_COUNTRY: 10,
  MAX_CONCURRENT: 20,
  
  // ========== 安全过滤 ==========
  EXCLUDE_COUNTRIES: ['RU', 'CN', 'KP', 'IR', 'SY'],
  BLACKLIST: ['192.168.', '10.', '172.16.', '127.', 'localhost'],
  
  // ========== 其他 ==========
  API_KEY: '',
  CACHE_TTL: 3600,
};
```

## 输出格式

### JSON（测试详情）

```json
{
  "totalProxies": 45,
  "testStats": {
    "lastTest": "2026-02-20T06:30:00.000Z",
    "duration": "35000ms",
    "totalFound": 500,
    "totalTested": 480,
    "validCount": 45,
    "failedCount": 435,
    "successRate": "9.4%"
  },
  "testDetails": [
    {
      "server": "1.2.3.4",
      "port": 8080,
      "latency": 120,
      "downloadSpeed": 5.2,
      "country": "US",
      "status": "ok"
    }
  ]
}
```

### Clash 订阅

```yaml
proxies:
  - name: "US-01"
    type: http
    server: 1.2.3.4
    port: 8080
    latency: 120
    speed: 5.2MB/s
```

## 目录结构

```
free-proxy-sub/
├── worker.js          # 主代码
├── wrangler.toml      # 部署配置
├── config.example.js  # 配置示例
├── README.md          # 说明文档
├── DEPLOY.md         # 部署指南
└── .gitignore
```

## 参考

- [TheSpeedX/PROXY-List](https://github.com/TheSpeedX/PROXY-List) - 代理数据源
- [Clash 配置](https://github.com/Dreamacro/clash/wiki/configuration)
- [Cloudflare Workers](https://workers.cloudflare.com/)
